    ...
    fu(i,1,n-1)
    {
        fu(j,i+1,n)
        {
            if(ans.second>distance(px[i],px[j]))
            {
                ...
                //cout<<"$$$$$ "<<px[i].x<<" "<<px[i].y<<" "<<px[j].x<<" "<<px[j].y<<" "<<ans.first.first<<" "<<ans.first.second<<endl;
            }
        }
    }
    return ans;
...
pair <pll,double> in_strip(point strip[],ll n,double min_d)
{
    ...
    fu(i,1,n-1)
    {
        fu(j,i+1,n)
        {
            if(strip[j].y-strip[i].y<min_d&&ans.second>distance(strip[i],strip[j]))
            {
...
int main()
{
    ...
    pl(ans.first.first-1);ps();pl(ans.first.second-1);ps();printf("%.6lf\n",ans.second);
    return 0;
}