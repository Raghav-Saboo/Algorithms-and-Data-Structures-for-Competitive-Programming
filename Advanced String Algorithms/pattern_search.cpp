//Problem link https://www.hackerearth.com/practice/data-structures/advanced-data-structures/suffix-arrays/practice-problems/algorithm/suffix-array-substring-occurrences/
#include <bits/stdc++.h>
...
int main() {
    ...
    //while(t--)
    {
        ...
        for (step = 1, cnt = 1; (cnt >> 1) < n; step++, cnt = cnt << 1) {
            ...
            //sort(Ls,Ls+n,cmp);
            rep(i, n) {
                Ps[step][Ls[i].index] = (i > 0 && Ls[i].val.fi == Ls[i - 1].val.fi && Ls[i].val.se == Ls[i - 1].val.se)
                                        ? Ps[step][Ls[i - 1].index] : i;
            }
            rep(i, n) {
                //  pl(Ls[i].val.fi);ps();pl(Ls[i].val.se);ps();ps();
            }
        ...
        while (low <= high) {
            mid = (low + high) / 2;
            ll rv = pat.compare(s.substr(Sa[mid], pat.size()));
            if (rv == 0) {
                foc = mid;
                //indpat.push_back(Sa[mid]);
                high = mid - 1;
