...
pair<string, ll> predicate(ll x, ll n, string s) {
    ll lower = lower_bound(counta, counta + n, x) - counta;
    return mp(s.substr(Sa[lower], x - counta[lower - 1] + lcparr[lower - 1]),
              x - counta[lower - 1] + lcparr[lower - 1]);
}
...
int main() {
    ...
    while (t--) {
        ...
        for (step = 1, cnt = 1; (cnt >> 1) < n; step++, cnt = cnt << 1) {
            ll lmx = 0;
            rep(i, n) {
                lmx = max(lmx, Ps[step - 1][i + cnt]);
            }
            rep(i, n) {
                Ls[i].index = i;
                Ls[i].val.fi = Ps[step - 1][i];
                Ls[i].val.se = (i + cnt) < n ? Ps[step - 1][i + cnt] : lmx + 1;
            }
            //pl(n);pn();
            radix_sort(n);
            //sort(Ls,Ls+n,cmp);
