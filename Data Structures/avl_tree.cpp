...
ll get_height(node *root) {
    ...
    return max((root->left) != NULL ? root->left->height : 0, (root->right) != NULL ? root->right->height : 0) + 1;
}
...
struct node *insert(struct node *root, ll val) {
    ...
    ll balance = (((root->left != NULL) ? root->left->height : 0) - ((root->right != NULL) ? root->right->height : 0));
